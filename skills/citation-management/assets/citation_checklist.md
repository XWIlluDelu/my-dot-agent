...
- [ ] Run: `python3 scripts/validate_citations.py references.bib --check-dois`
...
- [ ] Run: `python3 scripts/validate_citations.py references.bib`
...
- [ ] Run: `python3 scripts/format_bibtex.py references.bib --sort year`
...
```bash
python3 scripts/format_bibtex.py references.bib \
  --deduplicate \
  --sort year \
  --descending \
...
```bash
python3 scripts/validate_citations.py clean_references.bib \
  --check-dois \
...
```bash
python3 scripts/validate_citations.py clean_references.bib --verbose
```
...
# Re-extract metadata
python3 scripts/extract_metadata.py --doi CORRECT_DOI
```
...
```bash
# Extract from DOI
python3 scripts/extract_metadata.py --doi 10.xxxx/yyyy

# Or from PMID (biomedical)
python3 scripts/extract_metadata.py --pmid 12345678

# Or from arXiv
python3 scripts/extract_metadata.py --arxiv 2103.12345
```
...
# Then validate
python3 scripts/validate_citations.py fixed_references.bib
```
...
```bash
# From DOI
python3 scripts/doi_to_bibtex.py 10.1038/nature12345
...
python3 scripts/validate_citations.py references.bib --check-dois
