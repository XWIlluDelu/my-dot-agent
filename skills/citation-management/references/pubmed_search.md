...
**Basic search**:
```bash
python3 scripts/search_pubmed.py "diabetes treatment"
```
...
**Date range filter**:
```bash
python3 scripts/search_pubmed.py "CRISPR" \
...
```bash
python3 scripts/search_pubmed.py "cancer immunotherapy" \
...
**Export to BibTeX**:
```bash
python3 scripts/search_pubmed.py "Alzheimer's disease" \
  --limit 100 \
  --format bibtex \
...
# Run search
python3 scripts/search_pubmed.py --query-file query.txt --limit 500
```
...
for topic in "${TOPICS[@]}"; do
  python3 scripts/search_pubmed.py "$topic" \
    --limit 100 \
    --output "${topic// /_}.json"
  sleep 1
done
...
python3 scripts/search_pubmed.py "topic" --output results.json
...
3. **Validate retrieved citations**:
   ```bash
   python3 scripts/validate_citations.py pubmed_results.bib
   ```
