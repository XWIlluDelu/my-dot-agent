...
```bash
python3 scripts/validate_citations.py references.bib --check-required-fields
```
...
```bash
python3 scripts/validate_citations.py references.bib --check-authors
```
...
**Automated detection**:
```bash
python3 scripts/validate_citations.py references.bib --check-duplicates
```
...
```bash
python3 scripts/validate_citations.py references.bib --check-syntax
```
...
```bash
python3 scripts/validate_citations.py fixed_references.bib --verbose
```
...
```bash
# After extraction
python3 scripts/extract_metadata.py --doi ... --output refs.bib
python3 scripts/validate_citations.py refs.bib

# After manual edits
python3 scripts/validate_citations.py refs.bib

# Before submission
python3 scripts/validate_citations.py refs.bib --strict
```
