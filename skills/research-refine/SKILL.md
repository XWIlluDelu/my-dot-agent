...
User input (PROBLEM + vague APPROACH)
  -> Phase 0 (this agent): Freeze Problem Anchor
  -> Phase 1 (this agent): Scan grounding papers -> identify technical gap -> choose the sharpest route -> write focused proposal
  -> Phase 2 (External reviewer / REVIEWER_MODEL): Review for fidelity, specificity, contribution quality, and frontier leverage
  -> Phase 3 (this agent): Anchor check + simplicity check -> revise method -> rewrite full proposal
  ...
  -> Optional handoff: /research-refine-pipeline if you want the same proposal extended into a detailed experiment roadmap
```

## Constants

- **REVIEWER_MODEL = `gpt-5.4`** — Default reviewer model. Override via argument or use any available logged-in coding CLI or external backend.
...
- Default to **1-3 core experiment blocks** and leave the full execution roadmap to the experiment-planning stage inside `/research-refine-pipeline`.
...
Send the full proposal to the external reviewer (REVIEWER_MODEL) for an **elegance-first, frontier-aware, method-first** review. The reviewer should spend most of the critique budget on the method itself, not on expanding the experiment menu.
...
EOF
python scripts/reviewer_call.py \
...
`scripts/reviewer_call.py` now prefers logged-in coding CLIs:

- `codex exec`
- `claude -p`
- `gemini -p`

If no suitable CLI is available, it falls back to:

- `THIRD_PARTY_API_BASE` + `THIRD_PARTY_API_KEY`
- `OPENAI_API_KEY` with optional `OPENAI_BASE_URL` / `OPENAI_API_BASE`
- `GEMINI_API_KEY` or `GOOGLE_API_KEY`

Fallbacks if neither CLI nor API backend is available:

- Use any available external-model or MCP integration.
- Use a fresh subagent if the runtime supports subagents.
- If neither exists, run a strict self-review and label it clearly as a fallback so the user knows external review did not happen.

Use high reasoning effort. Prompt:
...
**CRITICAL: Persist reviewer continuity.** If using `scripts/reviewer_call.py`, reuse `refine-logs/reviewer-thread.json` for later rounds. If using another tool, reuse its thread or conversation handle when available.
...
Send the revised proposal back to the external reviewer in the **same conversation state** (reuse `refine-logs/reviewer-thread.json` or the tool's saved thread handle if supported):
...
- **Use high reasoning effort** for all external reviewer calls (pass `model_reasoning_effort: xhigh` or equivalent if supported).
- **Reuse the same reviewer state from Phase 2** (`refine-logs/reviewer-thread.json` or the tool's thread handle) for later rounds whenever possible.
...
3. `/research-refine-pipeline` adds the detailed claim-driven experiment roadmap when the method is ready
