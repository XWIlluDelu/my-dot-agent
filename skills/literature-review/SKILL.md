...
Visuals are strongly recommended when they make the review easier to scan or audit. If the current agent/runtime has access to a diagramming or image-generation workflow, add 1-2 figures such as:
...
- markdown tables
- ASCII flow diagrams
- clearly structured screening summaries
...
Access via direct API or a web-reading tool:
```python
