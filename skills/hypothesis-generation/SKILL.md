...
Visuals are strongly recommended when they clarify competing mechanisms, experimental design, or prediction structure. If the current agent/runtime has access to a diagramming or image-generation workflow, add 1-2 figures such as:
...
If no image-generation workflow is available, fall back to:

- ASCII diagrams
- compact markdown tables
- clearly structured bullet hierarchies
...
- Use a web-reading tool, direct API call, or downloaded page content to access PubMed literature
...
- Use search or literature APIs to find recent papers, preprints, and reviews
